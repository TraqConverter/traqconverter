      ...
      <section>
        ...
        >
          <div className="flex-1 min-w-[220px]">
            <div
              className="text-[11px] font-semibold tracking-[0.14em] mb-1"
              style={{ color: "#9a9178" }}
            >
              CUSTOM AMOUNT
            </div>
            <div className="text-sm" style={{ color: "#4a4638" }}>
              Need a specific number? Enter the credits you want and we&apos;ll create a Stripe
              checkout session for that exact total.
            </div>
          </div>
          <div className="flex items-center gap-2">
            <div
            ...
            >
              <input
                ...
                style={{ color: "#1f2a2e" }}
              />
              <span className="text-xs" style={{ color: "#8a8270" }}>credits</span>
            </div>
            <button
              type="button"
              onClick={() => handleBuyCredits(parseInt(customAmount, 10))}
              disabled={busy !== null || !customAmount}
              className="px-4 py-2.5 rounded-full text-sm font-semibold transition"
              style={{
                background: busy !== null || !customAmount ? "#9bc9c5" : "#0a7870",
                color: "#fff",
                cursor: busy !== null || !customAmount ? "not-allowed" : "pointer",
              }}
              onMouseEnter={(e) => {
                if (busy === null && customAmount) e.currentTarget.style.background = "#0a645d"
              }}
              onMouseLeave={(e) => {
                if (busy === null && customAmount) e.currentTarget.style.background = "#0a7870"
              }}
            >
              {busy?.startsWith("credits:") &&
              busy === `credits:${parseInt(customAmount, 10)}`
                ? "Redirecting…"
                : "Buy credits"}
            </button>
          </div>
